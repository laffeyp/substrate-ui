...
### 2026-09-23 — hardening (Sprints 058–062) + five failed caret pins + audit + Phase 8 opened

**What landed cleanly.** The 2026-09-22 code review named an untyped 2,632-line reveal.html with variables `S`, `KC`, `T`, `EV`, `CH`, `FQ`, `FTXT`, `G`, `A`, `B` — 49 raw envelope-kind literals — no ESLint config — no typechecking of the inline script. Sprint 058 extracted the class body to `web/reveal_component.ts` behind a `vite.config.ts:inlineDcScript` plugin (esbuild bundles + injects at HTML transform). Sprint 059 authored `web/vm/kinds.ts` with `EnvelopeKind`, `TranscriptRole`, `Surface`, `GraphMode`, `RevealLevel`, `GraphDirection`; 118 raw-string references converted. Sprint 060 executed the review's rename list. Sprint 062 landed a two-rule ESLint config (`id-length`, `no-restricted-syntax` blocking raw envelope-kind literals outside `kinds.ts`) wired into `npm run build`. A harness port-guard (`ServerHandle.start()` refuses if 8765 is bound) closed the stray-Python defect that had corrupted the 2026-09-22 shakeout. The parity gate expanded to scan `web/reveal_component.ts`.

**What five caret-in-place attempts taught.** The visual ask: click a tool card's caret, the header row does not move; the body opens below. The reveal shell had this simple property broken and could not get it fixed. Attempts and reverts:
1. Three-state toggle to override auto-open. Reverted — auto-open on `streamingShow` kept overriding.
2. `data-tool-row` + `getBoundingClientRect` + rAF `scrollTop` correction. Reverted — flashing, mis-targeted rows.
3. `overflow-anchor: none` + explicit scroll freeze. Reverted — refs still fought.
4. Memoized ref callbacks on `this`. Reverted — drift persisted; user reported still broken.
5. Full signal-driven Playwright probe (`TOOL_CARD_TOGGLED` vocab tag, headless assertion). Passed the assertion. The user reported the visible drift was still there. The probe measured a case the user was not exercising.

The common cause across all five: I never *measured* `scrollTop`, `header.top`, or `body_count_in_dom` at the four phases of a real click before authoring a fix. Every attempt was a hypothesis-driven code change. The first probe run, once written, took forty minutes and answered the question in under a second: state flips fine, DOM commits fine, `scroll_height` grows on open, `scroll_height` shrinks on close, `header_top` returns to origin — but only in the harness. The user's environment shows a different actor.

**The audit ruled a substrate change.** `process/planning/AUDIT-2026-09-23-dc-runtime-local-optimum.md` names five substrate properties that make the caret pin unachievable inside dc-runtime: fresh callback identities per render; `walkFor` keys by index (`web/dist/support.js:636`); no component boundaries; no `useLayoutEffect` surface; Chromium's default `overflow-anchor: auto` shifts `scrollTop`. Standard React lists solve this by construction. dc-runtime does not host that shape.

**Phase 8 opened.** `PLAN-2026-09-23-phase-8-transcript-atom-migration.md` — 428 lines, twenty-eight numbered references — migrates only the transcript subtree to a real React tree with `React.memo`-isolated rows, `useSyncExternalStore` subscription, atom-key `envelope.callId ?? seq:${envelope.seq}`, `useScrollAnchor` hook restoring an atom-relative offset. Six sprint cards (070–076) land on disk. Pixel baseline is the gate; SDD vocab lock unchanged at 30 tags. Electron unaffected — it loads `app/prototype-v7.html`, a separate shell; a follow-up sprint repoints Electron at the reveal shell after Phase 8 closes.

**Kit lesson (H19, new).** Discipline gates catch the wrong bug. Typecheck, lint, parity-gate, smoke, and the shakeout were all green through five failed caret attempts. What was missing was a *measurement* discipline: for behaviour-touching bugs, the observation contract must include a probe that records the specific quantity in question (here: header y and scrollTop across phases of the click) *before* any fix authoring. AGENTS.md's hard rule 9 (observation contract for behaviour-touching sprints) covers this in principle; the kit should teach the pattern more sharply — the probe is a first-class sprint output, not a debugging tool the agent may or may not reach for.

**Kit lesson (H20, new).** dc-runtime's execution model has a ceiling. Any UI requirement that demands stable component identity, per-atom memoization, or scroll math that survives a re-render hits the ceiling. The kit should carry this as a triage question when opening a UI-touching phase: does the template system host `React.memo` or an equivalent? If not, the phase's shape may be a substrate migration, not a code fix.
