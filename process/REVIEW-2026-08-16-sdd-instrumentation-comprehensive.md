...
*Reviewer role. Scope: whether substrate-ui has been fully and correctly SDD-instrumented across the console and the studio after Sprints 018–032. Targets read in full: `signals/versions/0.3.json` (742 lines), `signals/versions/0.3-rationale.md` (54 lines), `web/instrumentation/sdd.ts` (96 lines), `web/instrumentation/vocabulary.ts` (42 lines), `web/studio.ts` (288 lines), `tools/capture-grade.ts` (556 lines), `tools/check-vocabulary-parity.ts` (169 lines), `tools/sync-substrate-vocab.ts` (30 lines), `harness/capture_studio_signals.js` (65 lines), `process/WORKING_AGREEMENT.md` (127 lines), `process/ROADMAP-2026-08-16.md` (63 lines), sprint cards 030/031/032. Emit-site verification via `grep -oE '\bemit\("([A-Z_]+)"' web/app.ts web/studio.ts | sort -u`. New dated file per no-in-place-edits.*
