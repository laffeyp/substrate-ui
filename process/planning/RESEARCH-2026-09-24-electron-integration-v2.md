# Research — Electron and the reveal shell (v2)

*Opened 2026-09-24. First-principles pass on what Electron is, what our current code is, where the two touch, and what the shape of a real desktop shell for substrate-ui would be. v2 collapses v1's Open Questions section into Rulings — every question v1 raised has a best-practice / SDD / substrate-practice answer, so none is actually open.*

*v1: `RESEARCH-2026-09-24-electron-integration.md` (superseded by this file; kept on disk per hard rule 12).*
...
**Retire.** `electron/main.js` (rewrite), `electron/preload.js` (rewrite, much smaller), `bridge/main.py` (moves to `_deprecated/`), `bridge/vocab.py` (same), `app/prototype-v7.html` (same), `app/build.js`, `app/support.js`, `app/index.html`, `src/observability/bridge-ops.ts` (mirror of the retired op vocab).
...
v1 had a section here called "Open Questions" with six items. Every one has a best-practice answer under SDD + substrate discipline + shipping practice. Recorded here as the resolved shape.

**Python distribution — staged, cheapest first.** Sprint sequence: cheapest path (§4 option 1, require `uv` and `substrate` on the system) lands in one wrapping sprint and produces a working dev-dogfood .app. The bundled-interpreter path (§4 option 2, python-build-standalone) opens a separate epic when there is a v1 worth distributing externally. This is the SDD sweet-spot rule (≤2 files / one concept per sprint, hard rule 6) applied to shipping: the smallest wrapping that works before the signing pipeline that ships.

**Port strategy — dynamic in prod, 8765 in dev.** Electron spawns the server with `--port 0`, reads the bound port from the subprocess's first stdout line, `loadURL`s it. Dev keeps `8765` for the harness and the Playwright shakeout, which already pin that port. Best practice for a shipping desktop app is dynamic binding so multiple installs coexist and no user collides with a dev server on their machine.

**Multi-window — out of scope for the wrapping sprint.** Own epic. Every pane today is inside one BrowserWindow, driven by the reveal shell's pane grid. Native tear-off requires a real design pass on pane state, the pane registry, and per-window controllers — not a shim on top of the wrapper. Sequence it after the wrapping sprint closes.

**Auto-update — deferred.** `electron-updater` needs a cert and a feed and a release cadence. Not the first sprint. Set up when there is a v1 users hold that we need to push updates to. Same phasing as the bundled-Python pipeline.

**Windows and Linux — macOS first, cross-platform as follow-up epics.** Chromium renders the same on all three, so the renderer and preload don't fork; only packaging, entitlements, and update-feed shape do. macOS is the primary dev platform. Windows and Linux are separate packaging epics after the macOS wrapping sprint proves the shape. Best practice for one-team shipping: prove on one OS, port to others once the shape is stable.

**Retired trees — `_deprecated/electron-bridge-2026-09-12/`.** Hard rule 12: nothing gets deleted. `bridge/`, `app/prototype-v7.html`, `app/build.js`, `app/support.js`, `app/index.html`, `src/observability/bridge-ops.ts` all move under `_deprecated/electron-bridge-2026-09-12/` with an ARCHIVED note pointing at this doc and the commit that retired them.

The next artifact is a phase plan built on top of these rulings, not more questions.
