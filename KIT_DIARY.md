...
| H17 | Vocabulary-drift detection at the tag-name layer misses payload-shape drift; every ratified tag whose payload doesn't match Layer 2's `required` set is invented vocabulary by another name. | **confirmed 2026-09-10** | Two review passes on the same restart caught ~30 invented tag names (round 1) and ~15 invented payload shapes (round 2, Sprint 009 four-axis check). Same root: a mental model of the schemas that Layer 1 and Layer 2's actual ratifications didn't match. Fix: `tests/harness/payload-check.js` runs Layer 2's `required` set against every emit, standing across every sprint. |
| H18 | A sprint whose harness passes but whose card still reads `status: pending` and whose commit hasn't landed has closed the code, not the sprint; the audit trail is one-sided until all four artifacts (BLACKBOARD, card status, KIT_DIARY, commit) land. | **confirmed 2026-09-10** | Sprints 001–009 shipped with harnesses green and BLACKBOARD § Built entries, but every card read `status: pending`, this diary had no restart entry, and zero commits had landed. Future readers seeing the tree would have read the code as pre-close draft. Same-turn fix: flipped nine cards, added the entry above, staged the commit. |
| H19 | Vocabulary drift closes by codegen, not by discipline: any Layer-N identifier that lives in both the vocabulary JSON and the code needs a generated projection with a load-time drift guard, or the two copies drift silently until a review reads them side-by-side. | **confirmed 2026-09-11** | `scripts/gen-tags.mjs` writes `src/observability/tags.ts` from Layer 1's tag names + a runtime check that key count === JSON count; 50 raw emit-kind literals in the reducer became `Tag.X` in one sweep. Every emit is now compile-checked against Layer 1. |
| H20 | Layer 7 slot names drift by punctuation — a hyphen/underscore split between the JSON and the code compiles and passes tests until a codegen sweep runs. Discipline alone cannot see it; hand-maintained copies of vocabulary identifiers always diverge. | **confirmed 2026-09-11** | `scripts/gen-anchors.mjs` sweep surfaced the reducer's `header_popover` (underscore) vs Layer 7's `header-popover` (hyphen). The generated `PaneSlot.HEADER_POPOVER = "header-popover"` forces the vocabulary spelling; the shadow copy is deleted. Generalises H17/H19 from tag names to any Layer-N identifier. |
| H21 | A standing shared harness library beats per-sprint copies: any check that should run on every sprint's harness belongs in `tests/harness/lib/`, not restated per sprint. Once the library exists, a new class of assertion touches one file and every sprint inherits it. | **confirmed 2026-09-11** | commit `8e9f231` retrofitted all 19 sprint harnesses onto `lib/harness.js`. When `assertLayer5AllowedSet` landed later, every sprint inherited it. Sprint 048's tonal-checks reshape was the same shape earlier. |
| H23 | Sprint cards drift from Layer 2 more often than expected: cards drafted during the F1 vocabulary sweep encode a *proposed* decomposition; Layer 2's ratified `enum` and required-field sets are the actual contract, and the card is the shadow. Any close that reads back "vocabulary wins" without also updating the card leaves the shadow in the next reader's way. | **confirmed 2026-09-12** | Sprint 026's card said assay ordinal 128; Layer 7's `SURFACE_KIND_BYTES` said 3. Sprint 027's card said `STUDIO_VIEW_TOGGLED` was a four-way switcher; Layer 2's `to.enum` was `{form, canvas}`. Both closed by reading vocabulary; both left the card in place. Two sprints back-to-back is a class, not a coincidence. |
| H22 | Naming a hygiene lesson in the diary does not install it as a gate. H18's sprint-close discipline recurred on Sprints 018–024 within one day of being written. The four-artifact close needs a mechanical fixture (pre-commit hook or `npm run close-sprint <NNN>` helper) or discipline erodes at every close. | **surfaced 2026-09-11** (meta) | Commits `904faf7` through `334ff39` all shipped with the sprint card at `status: pending`, no `### Sprint NNN` paragraph under BLACKBOARD `## Built`, no KIT_DIARY entry. This checkpoint review's same-day fix flips the cards and lands the paragraphs; the fixture itself is TBD — H22 is the surface of the meta-class "diary entries are not gates." |
...
**H23: sprint cards drift from Layer 2 more often than expected.** Two consecutive sprints hit the same class. Sprint 026's card said the assay anchor byte was 128; Layer 7's `SURFACE_KIND_BYTES` encoded assay as ordinal 3. Sprint 027's card said `STUDIO_VIEW_TOGGLED` decomposed as `{trigger, view, route, termination}`; Layer 2's ratified `to.enum` was `{form, canvas}`. Both were drafted during the 2026-09-10 F1 vocabulary sweep, before Layer 2's per-tag `required` and `enum` fields were ratified against the card set. The close-time read of vocabulary beats the card in both cases — vocabulary is the contract; the card was the proposal. The current close protocol reads vocabulary first, which is correct, but leaves the card unrewritten and so the shadow lives on for the next reader. **Class**: the sprint card is a shadow of Layer 2 during F1 drafting; every close's first act should be to overwrite the card's ambiguous fields (payload counts, enum values, anchor bytes) with what vocabulary actually says, not leave the card as a historical draft. This is not new writing — this is bringing the shadow in line with the source. **Next-work class**: a `close-sprint` template that reads the card, greps the vocabulary for the tags it names, and offers the current Layer 2 payloads / Layer 7 bytes as a diff for the human to accept.

---

### 2026-09-11 — Checkpoint sprints 018-024: four lessons on codegen, slot punctuation, standing libraries, and diary-as-gate

**Context.** Sprints 018–024 landed the reveal/lens/delegate/descent/inspector arc (Epics E, F, and the opening of G) between 2026-09-10 and 2026-09-11. Seven commits shipped. Every harness green. `REVIEW-2026-09-11-checkpoint-sprints-001-024.md` grades the substrate as "the strongest sdd-kit-2 implementation on this project" with one load-bearing gap: sprint-close hygiene slipped on all seven sprints in exactly the class H18 named. Same-day fix: this diary entry, seven card flips, seven `### Sprint NNN` BLACKBOARD paragraphs, the two typed v0.2 proposals, six findings addressed.

**H19: vocabulary drift closes by codegen, not by discipline.** The user's live pushback ("if slot === 'descent' — again, these raw strings fucking everywhere") named the class. Fifty raw emit-kind literals lived in the reducer (`kind: "DELEGATE_CALL_FOLDED"` etc.) because Layer 1's 108 tag names were hand-copied at every emission site. Emitter's mouth-side validation caught malformed payloads but not a mistyped tag name — that only failed at harness time via `assertNoInventedTags`. Fix: `scripts/gen-tags.mjs` reads `signals/0.1.json § layer_1_lexical.tags` and writes `src/observability/tags.ts` with 108 entries + a load-time count-check that throws if the JSON is edited without regenerating. `package.json` `prebuild` runs the codegen. TypeScript now narrows `Tag.DELEGATE_CALL_FOLDED` to the literal string; a typo fails the compile. Sweep replaced 50 raw literals in the reducer, 3 in App.tsx + AnchorPainter.ts, and 6 in `reasons.ts`'s Layer 2 enum lookups. Grep confirms zero raw Layer 1 tag literals in `src/**/*.{ts,tsx}` outside `tags.ts`. **Class**: any Layer-N identifier that lives in both the JSON and the code needs a generated projection.

**H20: Layer 7 slot names drift by punctuation.** Sprint 024's `gen-anchors.mjs` sweep surfaced a real drift — the reducer had `header_popover` (underscore) while Layer 7 has `header-popover` (hyphen). The mismatch compiled and passed tests because no harness had yet claimed the header-popover anchor. `PaneSlot.HEADER_POPOVER = "header-popover"` from the generated file forces the vocabulary spelling; the shadow copy is deleted. **Class**: even a locked vocabulary drifts silently when two hand-maintained copies differ by punctuation.

**H21: a standing shared harness library beats per-sprint copies.** Commit `8e9f231` (2026-09-11) retrofitted all 19 sprint harnesses onto `tests/harness/lib/harness.js`. That commit's Layer 5 check was `pairing_ordering + forbidden_after` only. When the checkpoint review flagged the shallow assertion, `assertLayer5AllowedSet` landed in one file (`lib/layer5.js`) and every sprint inherited the widened check with no per-sprint edits. Second example this cycle: the F-1 tightening — payload-check.js's load-time assertion that Layer 1 ⊆ Layer 2 — closes at the library level and every sprint's `assertLayer2ShapesInTrace` call inherits it. **Class**: assertions that should run everywhere belong in the shared library, called from `lib/harness.js`'s scaffold.

**H22 (meta): naming a hygiene lesson in the diary does not install it as a gate.** H18 landed 2026-09-10. Sprints 018–024 shipped between 2026-09-10 and 2026-09-11 with all seven cards at `status: pending` and no `### Sprint NNN` paragraphs under `## Built`. The class returned within one day. Diary entries are not fixtures. A mechanical enforcement (pre-commit hook, `npm run close-sprint <NNN>` template that flips + drafts) is the next work; the diary entry alone is not the fix. **Class**: any discipline that should hold on every sprint needs a mechanical check at the sprint-close time, not a lesson written in a diary that authors then re-open one day later.

---

### 2026-09-10 — Restart + sprints 001-009: two lessons on vocabulary depth and sprint-close artifacts

**Context.** The 2026-09-08 Architect decision "substrate-ui is dead. Take the prototype and make it real" wiped every file outside the audit trail; sprints 001-009 rebuilt the shell on Electron + React + a Python bridge that imports substrate in-process (D62). Sprint 001 shipped the observability spine; sprints 002-005 built the pane grid (open, split, rearrange, close); sprints 006-009 built substrate binding (workspace picker, session create, driver probe, session resume). Every harness end-to-end green.

**H17: the vocabulary lock is the contract; drift shows up in payloads before it shows up in names.** Two reviews caught the same class of drift at different scales. Round 1 (`process/reviews/REVIEW-2026-09-10-epic-plan-and-sprint-cards.md`) caught ~30 invented tag NAMES across ~12 sprint cards — my emits referenced tags that Layer 1 v0.1 never ratified. Round 2 (the Sprint 009 four-axis check that arrived same-day) caught ~15 invented PAYLOAD SHAPES across the running code — the tag names matched Layer 1 but the payload fields didn't match Layer 2. Same root: I carried a mental model of the tag-and-field surface from Layer 0 review that Layer 1 and Layer 2's actual ratifications didn't match, and I never grepped the schemas before emitting. Class: hard rule 2 ("vocabulary is the contract; workers cannot invent tags") applies to the WHOLE schema (kind + required fields + property types), not just the string. Mechanical fix: `tests/harness/payload-check.js` reads Layer 2's schemas at run time and asserts every emit carries every required field; every sprint's e2e imports and calls it, so payload drift fails the sprint that introduces it — same standing-assertion pattern as the tonal-check reshape (Sprint 048's F6 fix).

**H18: sprint-close is four artifacts, not one.** The four-axis check surfaced an SDD hygiene gap: nine sprints of code shipped, nine harnesses green, but every card's frontmatter still read `status: pending`, `KIT_DIARY.md` had no restart entry, and no git commits marked any of the closes. The code was in the tree; the discipline was not. Class: sprint-close means four things — (1) the harness passes and the BLACKBOARD § Built entry lands, (2) the card's `status:` flips from `pending` to `closed` in the same commit as the code, (3) KIT_DIARY grows a numbered entry for any cross-sprint learning the close surfaced, (4) git commits the delta with a message naming the sprint id. Any absent step is a load-bearing gap even if the code runs; half-artifacts confuse the audit trail more than they help it. This diary entry, the nine `status: closed` flips, and the pending commit are the same-turn fix.

**Sub-lesson: `git show HEAD:<path>` before writing over an existing file.** I initially clobbered this diary's prior 278 lines with a fresh two-entry file. Hard rule 12 (no deletions of audit trail) applies to disk artifacts as strongly as it applies to sprint cards. The `feedback-no-in-place-edits-new-versions-only` memory says docs get new dated versions and prior versions stay on disk; for an append-only diary, the natural shape is to prepend a new dated entry, leaving every prior entry byte-for-byte intact. The Write tool doesn't enforce that — the discipline is on the author. When a Write target is a file that might already exist as an accumulating log (this diary, BLACKBOARD.md, ROADMAP.md), read it first, and prefer Edit-style prepend/append over a full rewrite.
