...
_TOPO_NAME_RX = re.compile(r"^[a-z0-9_]+$")


def op_topology_validate(payload: dict) -> dict:
    """Sprint 027 — validate a Studio topology draft against Layer 2's
    STUDIO_VALIDATED payload shape. Returns
        {topo_name, producer_count, view_count, trigger_count, route_count}
    on success or {__error__:true, errors:[...]} on failure. Validation
    is name-shape + numeric-plausibility only under v0.1; a later widening
    sprint will import the substrate kernel and re-run its own validators.
    """
    ...
    def _nonneg_int(k: str, minimum: int) -> int | None:
        v = payload.get(k)
        return v if isinstance(v, int) and v >= minimum else None
    producer_count = _nonneg_int("producer_count", 1)
    if producer_count is None:
        return {"__error__": True, "errors": [
            "producer_count must be an integer ≥ 1 (a topology has at least one producer)",
        ]}
    view_count = _nonneg_int("view_count", 0) or 0
    trigger_count = _nonneg_int("trigger_count", 0) or 0
    route_count = _nonneg_int("route_count", 0) or 0
    return {
        "topo_name": topo_name,
...
def op_topology_build(payload: dict) -> dict:
    """Sprint 027 — write a stub topology file to
    ~/.substrate/topologies/<topo_name>.py, then return
    {topo_name, record_root} per Layer 2's STUDIO_BUILT payload. The
    stub carries a `topology(...)` factory so a later sprint can hand
    it to substrate.api.TopologyBuilder; v0.1's build produces the
    file, not the run. Failure surfaces STUDIO_BUILD_REJECTED with a
    typed errors list.
    """
    topo_name = payload.get("topo_name", "")
    if not isinstance(topo_name, str) or not _TOPO_NAME_RX.fullmatch(topo_name):
    ...
    topo_dir = Path.home() / ".substrate" / "topologies"
    topo_dir.mkdir(parents=True, exist_ok=True)
    path = topo_dir / f"{topo_name}.py"
    stub = (
        f"# {topo_name}.py — Studio-authored topology stub (Sprint 027)\n"
        f"# generated by substrate-ui's Studio surface. A later widening\n"
        f"# sprint parses this file back into substrate.api.TopologyBuilder.\n"
        "from substrate import api\n\n\n"
        "def topology(b: api.TopologyBuilder) -> None:  # noqa: D401\n"
        "    raise NotImplementedError('Studio draft — form-authored')\n"
    )
...
def _handle_short_op(op: BridgeOp, msg: dict, rid: str) -> None:
    ...
    if op is BridgeOp.LIST_ASSAYS:
        try:
            ...
            reply_err(rid, f"{SCR.REGISTRY_ERROR.value}:{e}")
        return
    if op is BridgeOp.TOPOLOGY_VALIDATE:
        try:
            result = op_topology_validate(msg)
            if result.get("__error__"):
            ...
            reply_err(rid, f"{SCR.REGISTRY_ERROR.value}:{e}")
        return
    if op is BridgeOp.TOPOLOGY_BUILD:
        try:
            result = op_topology_build(msg)
            if result.get("__error__"):
                reply_err(rid, json.dumps({"errors": result.get("errors", [])}))
            else:
                reply_ok(rid, result)
