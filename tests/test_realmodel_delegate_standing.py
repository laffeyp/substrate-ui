"""Sprint 052 (4) — live-model standing sub-agent end-to-end.
...
back to the parent as a tool result. Context comes from the sub-agent's
OWN record, not from the parent's per-call injection.

This test proves the whole loop with two real models:

  1. Reviewer session runs `qwen2.5:7b-instruct` with a role seed that
     gives it a fixed identity ("your name is REVIEWER-42, reply with
     just that name when asked").
  2. Reviewer's first turn opens the session on disk (the seed is what
     it knows).
  3. Parent session runs `qwen2.5:7b-instruct` too, with the delegate
     tool wired to the same SessionRegistry.
  4. Parent is asked to delegate to `reviewer` and quote what the
     reviewer said.
  5. Assertions: the reviewer's record grows with a second UserMessage
     containing the parent's task; the delegate ToolResult carries
     `via="standing_session:reviewer"`; the parent's FinalAnswer
     quotes `REVIEWER-42`.

Both models are real. Both records are on tmp_path. Gated by
`@pytest.mark.realmodel`; skipped when Ollama or qwen2.5:7b-instruct
is absent."""
...
def _require_model() -> None:
    ...
    if _DRIVER not in ids:
        pytest.skip(f"live standing-session skipped — model absent: {_DRIVER}")
...
def _reviewer_factory(
    ...
    def factory(manifest: Any, first_turn_user_message: Any = None) -> Any:
        ...
        return session_topology(
            driver=OllamaResponder(_DRIVER, max_tokens=32, temperature=0),
            driver_name=_DRIVER,
...
async def test_live_standing_reviewer_answers_and_parent_quotes(tmp_path: Path) -> None:
    ...
    delegate = make_delegate(
        responder=OllamaResponder(_DRIVER, max_tokens=64, temperature=0),
        root=parent_workspace,
        session_registry=registry,
        parent_session_id="s_parent",
    )

    # Scripted parent: the model does NOT get to decide whether to pass
    # `child_session_name` — the script forces the correct kwarg. What
    # this test verifies is the WIRE (delegate path 1 → standing session
    # → reviewer answers → parent reads the answer back), not whether a
    # small model reliably remembers an optional kwarg. Qwen 2.5-7b in
    # our probe left `child_session_name` off ~100% of the time even
    # with an explicit prompt; separate ergonomic problem, separate fix.
    parent_factory = session_topology(
        driver=OllamaResponder(
            _DRIVER,
            max_tokens=200,
            temperature=0,
            system="You have access to the delegate tool. Quote what the tool returns.",
        ),
        driver_name=_DRIVER,
        driver_context_tokens=8192,
        seed="",
        ...
        script=[
            (
                "delegate",
                [{"task": "what is your name?", "child_session_name": "reviewer"}],
            )
        ],
        first_turn_user_message=UserMessage(
            text=(
                "Delegate to the reviewer session to ask its name. "
                "Then tell me exactly what the reviewer said."
            ),
            turn_index=0,
            assembled_prompt=(
                "Delegate to the reviewer session to ask its name. "
